static const BYTE ms_execute_indirect_state_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE ms_execute_indirect_state_dxil = { ms_execute_indirect_state_code_dxil, sizeof(ms_execute_indirect_state_code_dxil) };
