static const BYTE as_execute_indirect_state_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE as_execute_indirect_state_dxil = { as_execute_indirect_state_code_dxil, sizeof(as_execute_indirect_state_code_dxil) };
