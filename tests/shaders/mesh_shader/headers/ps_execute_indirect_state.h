static const BYTE ps_execute_indirect_state_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE ps_execute_indirect_state_dxil = { ps_execute_indirect_state_code_dxil, sizeof(ps_execute_indirect_state_code_dxil) };
