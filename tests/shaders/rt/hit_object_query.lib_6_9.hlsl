...
void RayGen()
{
	...
	RayQuery<RAYQUERY_FLAG_NONE> rq;
	rq.TraceRayInline(RTAS, 0, 0x1, ray);

	BuiltInTriangleIntersectionAttributes attr = (BuiltInTriangleIntersectionAttributes)0;

	while (rq.Proceed())
	{
		if (rq.CandidateType() == CANDIDATE_NON_OPAQUE_TRIANGLE)
		{
			if (rq.CandidateGeometryIndex() & 1)
	...
	// Miss becomes Nop in RQ, explicitly make a miss object.
	// Make sure that RQ -> Miss doesn't happen.
	if (hit.IsMiss())
	...
	if (hit.IsHit())
		hit.SetShaderTableIndex(rq.CommittedGeometryIndex() + 2 * rq.CommittedInstanceIndex());

	// Now, the results should match the normal TraceRay paths.
