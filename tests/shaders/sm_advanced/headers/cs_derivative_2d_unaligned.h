static const BYTE cs_derivative_2d_unaligned_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE cs_derivative_2d_unaligned_dxil = { cs_derivative_2d_unaligned_code_dxil, sizeof(cs_derivative_2d_unaligned_code_dxil) };
