static const BYTE vs_helper_lane_wave_ops_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE vs_helper_lane_wave_ops_dxil = { vs_helper_lane_wave_ops_code_dxil, sizeof(vs_helper_lane_wave_ops_code_dxil) };
