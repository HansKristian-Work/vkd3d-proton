static const BYTE cs_cbv_layout_modern_uint32_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE cs_cbv_layout_modern_uint32_dxil = { cs_cbv_layout_modern_uint32_code_dxil, sizeof(cs_cbv_layout_modern_uint32_code_dxil) };
