static const BYTE cs_quad_swap_2d_unaligned_code_dxil[] =
{
    ,
};
VKD3D_UNUSED static const D3D12_SHADER_BYTECODE cs_quad_swap_2d_unaligned_dxil = { cs_quad_swap_2d_unaligned_code_dxil, sizeof(cs_quad_swap_2d_unaligned_code_dxil) };
