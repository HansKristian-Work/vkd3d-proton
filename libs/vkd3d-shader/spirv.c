...
enum vkd3d_spirv_extension
{
    ...
    VKD3D_SPV_FROG_RAW_ACCESS_CHAINS            = 0x00000080,
};
...
static const struct vkd3d_spirv_extension_info vkd3d_spirv_extensions[] =
{
    ...
    {VKD3D_SPV_FROG_RAW_ACCESS_CHAINS,          "SPV_FROG_raw_access_chains"},
};
...
static const struct vkd3d_spirv_capability_extension_mapping vkd3d_spirv_capability_extensions[] =
{
    ...
    {SpvCapabilityRawAccessChainsFROG,                    VKD3D_SPV_FROG_RAW_ACCESS_CHAINS},
};
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    resource_symbol.info.resource.raw = false;
    resource_symbol.info.resource.ssbo = false;
    resource_symbol.info.resource.coherent = false;
        ...
        unsigned int structure_stride, bool raw)
{
    ...
    if (binding && (binding->flags & VKD3D_SHADER_BINDING_FLAG_BINDLESS))
    {
        ...
        var_id = global_binding->var_id;
        use_coherent = !!(flags & VKD3D_SHADER_GLOBAL_BINDING_COHERENT);
    }
    else if (binding && (binding->flags & VKD3D_SHADER_BINDING_FLAG_RAW_VA))
    {
        unsigned int flags = 0;
    ...
    resource_symbol.info.resource.coherent = use_coherent;
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    const struct vkd3d_shader_src_param *src = instruction->src;
    SpvMemoryAccessMask access_mask = SpvMemoryAccessMaskNone;
    uint32_t u32_type_id, type_id, ptr_id, ptr_type_id;
    ...
    for (i = 0; i < VKD3D_VEC4_SIZE; ++i)
    {
        if (dst->write_mask & (VKD3DSP_WRITEMASK_0 << i))
        {
            ...
            component_count = max(component_count, component_idx + 1);
        }
    }

    u32_type_id = vkd3d_spirv_get_type_id(builder, VKD3D_TYPE_UINT, 1);
    type_id = vkd3d_spirv_get_type_id(builder, VKD3D_TYPE_UINT, component_count);
    ...
    ptr_id = vkd3d_spirv_build_op_trv(builder, &builder->function_stream,
            SpvOpRawAccessChainFROG, ptr_type_id, indices, ARRAY_SIZE(indices));
    texel_id = vkd3d_spirv_build_op_loadv(builder, type_id, ptr_id, access_mask, &alignment, 1);
    ...
    for (i = 0, j = 0; i < VKD3D_VEC4_SIZE; ++i)
    {
        ...
        component_idx = vkd3d_swizzle_get_component(resource->swizzle, i);
        if (component_count == 1)
            constituents[j++] = texel_id;
        else
            constituents[j++] = vkd3d_spirv_build_op_composite_extract(builder, u32_type_id, texel_id, &component_idx, 1);
    }
        ...
        const struct vkd3d_shader_instruction *instruction)
{
    ...
    ptr_id = vkd3d_spirv_build_op_trv(builder, &builder->function_stream,
            SpvOpRawAccessChainFROG, ptr_type_id, indices, ARRAY_SIZE(indices));
    vkd3d_spirv_build_op_storev(builder, ptr_id, val_id, access_mask, &alignment, 1);
