...
static void vkd3d_timestamp_profiler_resolve_timestamp(struct vkd3d_timestamp_profiler *profiler,
        const struct vkd3d_timestamp_profiler_submitted_work *work)
{
    ...
    VK_CALL(vkGetQueryPoolResults(profiler->device->vk_device, profiler->timestamp_pool,
            work->timestamp_index * 2, 2, sizeof(tses), tses, sizeof(tses[0]),
    ...
    switch (work->pipeline_type)
    {
        case VKD3D_PIPELINE_TYPE_GRAPHICS:
            VK_CALL(vkGetQueryPoolResults(profiler->device->vk_device, profiler->vs_invocation_pool,
        ...
        case VKD3D_PIPELINE_TYPE_COMPUTE:
            VK_CALL(vkGetQueryPoolResults(profiler->device->vk_device, profiler->cs_invocation_pool,
        ...
        case VKD3D_PIPELINE_TYPE_MESH_GRAPHICS:
            ...
            {
                VK_CALL(vkGetQueryPoolResults(profiler->device->vk_device, profiler->ms_invocation_pool,
...
static void vkd3d_timestamp_profiler_flush(struct vkd3d_timestamp_profiler *profiler)
{
    ...
    fprintf(file, "PSO Type,PSO Hash,Shader Hashes,Total Time (s),Non-PS invocations,PS invocations,Commands\n");
    ...
    for (i = 0; i < profiler->pso_states_count; i++)
    {
        ...
        fprintf(file, "%s,%016"PRIx64",%s,%.9f,%"PRIu64",%"PRIu64",%"PRIu64"\n",
...
static void *vkd3d_timestamp_profiler_thread(void *arg)
{
    ...
    for (;;)
    {
        ...
        while (profiler->read_progress < write_progress)
        {
            ...
            vkd3d_timestamp_profiler_resolve_timestamp(profiler, work);
