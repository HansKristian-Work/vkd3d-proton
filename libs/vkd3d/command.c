        ...
        VkAccessFlags2 outside_access, VkImageMemoryBarrier2 *barrier)
{
    ...
    if (path == VKD3D_RESOLVE_IMAGE_PATH_DIRECT)
    {
        ...
        resolve_access = unified ? 0 : VK_ACCESS_2_TRANSFER_WRITE_BIT;
    }
        ...
        const VkImageResolve2 *resolve, DXGI_FORMAT format, D3D12_RESOLVE_MODE mode)
{
    ...
    /* If there are layout transitions, srcAccessMask must not be 0, so we can rely on this check. */
    if (vk_image_barriers[0].srcAccessMask || vk_image_barriers[1].srcAccessMask)
        VK_CALL(vkCmdPipelineBarrier2(list->cmd.vk_command_buffer, &dep_info));
    else
    {
        d3d12_command_list_register_pending_transfer_image_write(
