...
{
    /* There is no RESOURCE_STATE_DEPTH_READ equivalent in D3D12, only LAYOUT_DEPTH_STENCIL_READ,
     * which presumably implies shader reads as well.
     * The D3D12 docs are misleading and imply the layout only maps to D3D12_RESOURCE_STATE_DEPTH_READ,
     * but legacy DEPTH_READ does not allow resource reads on its own. */
        ...
        const D3D12_BARRIER_SUBRESOURCE_RANGE *range)
{
    ...
    else
    {
        ...
        if (range->IndexOrFirstMipLevel == 0 && range->FirstArraySlice == 0 &&
                ...
                range->NumArraySlices >= d3d12_resource_desc_get_layer_count(&resource->desc))
        {
            for (i = 0; i < range->NumPlanes; i++)
