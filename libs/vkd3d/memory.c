        ...
        VkMemoryPropertyFlags type_flags, struct vkd3d_device_memory_allocation *allocation)
{
    ...
    /* Internal memory allocations should not spuriously fail due to budget.
     * We really want them to be allocated even if we have exceeded budget. */
    if (FAILED(hr = vkd3d_allocate_device_memory(device, memory_requirements.size,
            type_flags, memory_requirements.memoryTypeBits, NULL, false, allocation)))
        return hr;
    ...
    if (FAILED(vr = VK_CALL(vkBindImageMemory2(device->vk_device, 1, &bind_info))))
        return hresult_from_vk_result(vr);
