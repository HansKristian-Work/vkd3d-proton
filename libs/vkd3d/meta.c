        ...
        VkPipeline *pipeline)
{
    ...
    if (layout == VK_NULL_HANDLE)
    {
        ...
        mapping[0].sourceData.pushIndex.pushOffset = VKD3D_DESCRIPTOR_HEAP_META_PUSH_DATA_OFFSET;
        mapping[0].sourceData.pushIndex.heapArrayStride = device->bindless_state.cbv_srv_uav_size;
        mapping[0].sourceData.pushIndex.heapIndexStride = device->bindless_state.cbv_srv_uav_size;
        ...
        mapping[1].sourceData.pushIndex.heapOffset = device->bindless_state.packed_raw_buffer_offset;
        ...
        const VkSpecializationInfo *spec_info, bool descriptor_buffer_compatible, VkPipeline *vk_pipeline)
{
    ...
    if (layout == VK_NULL_HANDLE)
    {
        ...
        mapping[0].sourceData.pushIndex.heapOffset = 0; /* We account for any red-zone ourselves. */
        mapping[0].sourceData.pushIndex.pushOffset = VKD3D_DESCRIPTOR_HEAP_META_PUSH_DATA_OFFSET;
        mapping[0].sourceData.pushIndex.heapArrayStride = meta_ops->device->bindless_state.cbv_srv_uav_size;
        mapping[0].sourceData.pushIndex.heapIndexStride = meta_ops->device->bindless_state.cbv_srv_uav_size;
