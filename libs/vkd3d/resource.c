        ...
        D3D12_QUERY_HEAP_FLAGS flags, struct d3d12_query_heap **heap)
{
    ...
    if (!d3d12_query_heap_type_is_inline(desc->Type))
    {
        ...
        /* We won't have the time to reset the query pool, but don't gate
         * support of ID3D12Device15 on this. Applications are not supposed to resolve queries
         * before the queries are done. */
    ...
    else
    {
        ...
        if (FAILED(hr) && (flags & D3D12_QUERY_HEAP_FLAG_CPU_RESOLVE))
        {
            /* Technically, cached | coherent isn't guaranteed (but in reality any relevant impl supports it).
             * Prefer coherent over cached since it's plausible that apps might spin. */
            memory_properties &= ~VK_MEMORY_PROPERTY_HOST_CACHED_BIT;
            hr = vkd3d_allocate_internal_buffer_memory(
                device, object->vk_buffer, memory_properties, alloc_flags, &object->device_allocation);
        }
