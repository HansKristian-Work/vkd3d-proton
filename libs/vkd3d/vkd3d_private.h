...
struct d3d12_device
{
    ...
    ID3D12Device10Vtbl device_vtable_patched;
};
...
static inline uint32_t d3d12_desc_heap_offset_from_gpu_handle(struct d3d12_device *device,
        D3D12_GPU_DESCRIPTOR_HANDLE handle)
{
    uint32_t low_word = (uint32_t)handle.ptr;

    /* Could use root signature to deduce the divisor, but that seems a little more likely to go wrong
     * in case apps do weird things like setting tables before updating root signature.
     * Encode this information in the upper 32-bit GPU VA instead. */
    if (d3d12_device_use_embedded_mutable_descriptors(device))
    {
        if (handle.ptr & VKD3D_RESOURCE_EMBEDDED_RESOURCE_HEAP_MASK)
            return low_word >> device->bindless_state.descriptor_buffer_cbv_srv_uav_size_log2;
        else
            return low_word >> device->bindless_state.descriptor_buffer_sampler_size_log2;
    }
    else
        return low_word / VKD3D_RESOURCE_DESC_INCREMENT;
}
